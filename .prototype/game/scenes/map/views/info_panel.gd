...
# Label settings for dynamic styling
var _h1_label_settings: LabelSettings
var _h2_label_settings: LabelSettings
var _caption_label_settings: LabelSettings
...
func load_body(body_id: String) -> void:
	...
	# Reapply label settings after loading data
	if _caption_label_settings or _value_label_settings:
		_apply_settings_to_data_points()
...
func apply_label_settings(h1: LabelSettings, h2: LabelSettings, caption: LabelSettings, value: LabelSettings) -> void:
	_h1_label_settings = h1
	_h2_label_settings = h2
	_caption_label_settings = caption
	_value_label_settings = value
	
	# Apply to header labels
	if _name_label and _h1_label_settings:
		_name_label.label_settings = _h1_label_settings
	if _type_label and _h2_label_settings:
		_type_label.label_settings = _h2_label_settings
	if _subtype_label and _h2_label_settings:
		_subtype_label.label_settings = _h2_label_settings
	
	# Apply to all data point displays
	_apply_settings_to_data_points()


func _apply_settings_to_data_points() -> void:
	# Collect all DataPointDisplay nodes
	var data_points = []
	
	# Orbit data points
	data_points.append_array([_dp_aphelion, _dp_parent, _dp_perihelion, _dp_period, 
		_dp_semi_major_axis, _dp_eccentricity, _dp_avg_orbital_speed])
	
	# Physics data points
	data_points.append_array([_dp_radius, _dp_gravity])
	
	# Apply settings to each data point
	for dp in data_points:
		if dp and dp.has_method("set_caption_label_settings") and _caption_label_settings:
			dp.set_caption_label_settings(_caption_label_settings)
		if dp and dp.has_method("set_value_label_settings") and _value_label_settings:
			dp.set_value_label_settings(_value_label_settings)
