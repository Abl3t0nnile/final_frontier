...
SolarSystem.get_body_positions_at_time(ids: Array[String], sst: float) -> Dictionary  # Batch — { id → Vector2 }, beliebiger Zeitpunkt, keine Seiteneffekte
SolarSystem.get_body_position_at_time(id: String, sst: float) -> Vector2           # Einzelkörper-Wrapper um get_body_positions_at_time
...
- **`get_body_positions_at_time` (Batch):** Berechnet Weltpositionen einer beliebigen Menge von Körpern für einen beliebigen `sst`-Wert ohne `_current_state` zu verändern. Löst intern alle benötigten Vorfahren auf und macht einen einzigen Durchlauf durch `_update_order`. Bei 200+ Körpern in der Simulation ist die Batch-Variante deutlich effizienter als wiederholte Einzelaufrufe, da die Vorfahren-Kette nicht pro Körper neu aufgebaut wird. `get_body_position_at_time` ist ein dünner Wrapper für den Einzelfall. Ausschließlich für den Vorspul-Modus der StarChart vorgesehen — nur für sichtbare Körper aufrufen.
