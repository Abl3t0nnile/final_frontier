...
func _draw() -> void:
	...
	var col: Color     = belt_def.color_rgba
	...
	var vis_min := cam_local - vp_half - Vector2(margin, margin)
	var vis_max := cam_local + vp_half + Vector2(margin, margin)
	...
	for i in _points_km.size():
		...
		var alpha: float = col.a * lerpf(0.08, 1.0, w)
		draw_circle(px, size, Color(col.r, col.g, col.b, alpha))
