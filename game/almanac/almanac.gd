...
@onready var _fwd_btn: Button = %ForwardButton
@onready var _title_label: Label = %TitleLabel
@onready var _article_title: Label = %Article/Header/TitleLabel
@onready var _summary_text: RichTextLabel = %SummaryText
@onready var _hero_image: TextureRect = %Hero
@onready var _overview_panel: VBoxContainer = %Panel

# Data Panels
@onready var _orbit_data: VBoxContainer = %OrbitData
@onready var _physics_data: VBoxContainer = %PhysicsData
@onready var _atmo_data: VBoxContainer = %AthmoData
...
func _display_article(article_id: String) -> void:
	...
	$VBox/ContentBox.scroll_to_line(0)
...
func _show_body_article(id: String) -> void:
	...
	# Sections aus AlmanachContentComponent
...
func _build_body_overview(obj: GameObject) -> void:
	...
	if content and not content.image.is_empty():
		var tex := load(content.image)
		_hero_image.texture = tex if tex else null
		_hero_image.visible = tex != null
	else:
		_hero_image.visible = false
...
func _build_orbit_data(def: BodyDef) -> void:
	...
	match motion.model:
		"kepler2d":
			...
			_add_unit_value(grid, "Große Halbachse", "%.4f" % SpaceMath.km_to_au(a), "AU")
			...
			if not is_zero_approx(vel):
				_add_unit_value(grid, "Ø Geschwindigkeit", "%.2f" % vel, "km/s")
...
func _build_physics_data(def: BodyDef) -> void:
	...
	_add_unit_value(grid, "Masse", _format_mass(mass), "kg")
	
	var density := SpaceMath.body_density_g_cm3(mu, r)
	if not is_zero_approx(density):
		_add_unit_value(grid, "Dichte", "%.2f" % density, "g/cm³")
	
	var gravity := SpaceMath.surface_gravity_ms2(mu, r)
	if not is_zero_approx(gravity):
		_add_unit_value(grid, "Schwerkraft", "%.2f" % gravity, "m/s²")
	
	var v_esc := SpaceMath.escape_velocity_km_s(mu, r)
	if not is_zero_approx(v_esc):
		_add_unit_value(grid, "Fluchtgeschwindigkeit", "%.2f" % v_esc, "km/s")


func _build_atmo_data(def: BodyDef, content: AlmanachContentComponent) -> void:
	# Nur anzeigen wenn Infobox-Daten vorhanden
	...
	# Bestehende Grids löschen außer Header
	for child in vbox.get_children():
		if child.name != "Label" and child.name != "HSeparator2":
			child.queue_free()
	...
	if atmo.has("pressure"):
		_add_unit_value(grid, "Druck", str(atmo["pressure"]), "bar")
	...
	if atmo.has("composition"):
		...
		for comp_name: String in composition:
			_add_unit_value(comp_grid, comp_name, str(composition[comp_name]), "%")
...
func _build_body_sections(obj: GameObject) -> void:
	...
	# Bestehende Sections löschen (nach Overview Panel)
	var child_idx := _article.get_children().find(_overview_panel) + 1
	while child_idx < _article.get_child_count():
		_article.get_child(child_idx).queue_free()
		child_idx += 1
	
	# Sections aus Component rendern
...
func _clear_article_content() -> void:
	...
	# Sections löschen (außer Overview)
	var child_idx := _article.get_children().find(_overview_panel) + 1
	while child_idx < _article.get_child_count():
		_article.get_child(child_idx).queue_free()
		child_idx += 1
...
func _on_link_clicked(meta: Variant) -> void:
	...
	if link.begins_with("concept:"):
		var concept_id := link.right(link.length() - 8)
		open_concept(concept_id)
	elif link.begins_with("body:"):
		var body_id := link.right(link.length() - 5)
		open_body(body_id)
